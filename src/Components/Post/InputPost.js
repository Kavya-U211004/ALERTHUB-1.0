...
// Add state to track the post ID to be updated or deleted
const [postIdToUpdate, setPostIdToUpdate] = useState(null);

// Function to handle updating a post
const handleUpdatePost = async (postId) => {
  // Fetch the post data to pre-populate the input fields
  try {
    const response = await axios.get(`http://localhost:5000/api/posts/${postId}`);
    const postData = response.data;
    setIssue(postData.content);
...
// Function to handle submitting an updated post
const handleUpdateSubmit = async () => {
  try {
    ...
    formData.append('pincode', pincode);
    formData.append('address', address);
    const response = await axios.put(`http://localhost:5000/api/posts/${postIdToUpdate}`, formData, {
